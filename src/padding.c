...
int PadImage(unsigned char* image, unsigned char* imageLayer, 
			 int imageWidth, int imageHeight, int padding) // image: the input image, imageLayer:  the output image with padding, channel = 3 for RGB
	{
		int i,j;
		int imgW;
		int imgH;
		imgW= imageWidth+padding;
